...
```
CONTENT_ID=xxx
curl -X GET "${ATLASSIAN_CONFLUENCE_URL}/wiki/rest/api/analytics/content/$CONTENT_ID/views" \
  -u $CONFLUENCE_ACCESS_EMAIL_AND_TOKEN \
  -H 'Accept: application/json'
```
...
```
curl -X GET "${ATLASSIAN_CONFLUENCE_URL}/wiki/rest/api/analytics/content/$CONTENT_ID/viewers" \
  -u $CONFLUENCE_ACCESS_EMAIL_AND_TOKEN \
  -H 'Accept: application/json'
```

### Get [Page with likes](https://developer.atlassian.com/cloud/confluence/rest/v2/api-group-page/#api-pages-id-get) (v2)

```
curl -X GET "${ATLASSIAN_CONFLUENCE_URL}/wiki/api/v2/pages/$CONTENT_ID?include-likes=true" \
  -u $CONFLUENCE_ACCESS_EMAIL_AND_TOKEN \
  -H 'Accept: application/json'
```
